...
export class PopupManager {
  ...
  showAddSuccess(customMessage?: string) {
    toast.success(
      customMessage || this.messages.success?.add || 'Added successfully!'
    );
  }

  showUpdateSuccess(customMessage?: string) {
    toast.success(
      customMessage || this.messages.success?.update || 'Updated successfully!'
    );
  }

  showDeleteSuccess(customMessage?: string) {
    toast.success(
      customMessage || this.messages.success?.delete || 'Deleted successfully!'
    );
  }
  ...
  showAddError(error?: unknown, customMessage?: string) {
    const message =
      customMessage || this.messages.error?.add || 'Failed to add item';
    toast.error(this.getErrorMessage(error, message));
  }

  showUpdateError(error?: unknown, customMessage?: string) {
    const message =
      customMessage || this.messages.error?.update || 'Failed to update item';
    toast.error(this.getErrorMessage(error, message));
  }

  showDeleteError(error?: unknown, customMessage?: string) {
    const message =
      customMessage || this.messages.error?.delete || 'Failed to delete item';
    toast.error(this.getErrorMessage(error, message));
  }

  showValidationError(message?: string) {
    toast.error(
      message || this.messages.error?.validation || 'Please check your input'
    );
  }
  ...
  async confirmDelete(customMessage?: string): Promise<boolean> {
    const message =
      customMessage ||
      this.messages.confirmation?.delete ||
  ...
  async confirmBulkDelete(
    ...
    const message =
      customMessage ||
      this.messages.confirmation?.bulkDelete?.replace(
        '{count}',
        count.toString()
      ) ||
  ...
  private getErrorMessage(error: unknown, fallback: string): string {
    if (error instanceof Error) {
      ...
      if (
        ...
        return (
          this.messages.error?.network || 'Network error. Please try again.'
        );
      }
...
export function createPopupManager(
  ...
  const typeTranslations =
    type === 'expense' ? translations.expenses : translations.income;
