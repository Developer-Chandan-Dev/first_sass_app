...
export const fetchExpenses = createAsyncThunk<
  ...
  {
    expenseType: 'free' | 'budget' | 'all';
    filters: ExpenseFilters;
    page: number;
    pageSize: number;
  }
>('expenses/fetchExpenses', async (params) => {
...
>('expenses/addExpense', async (expenseData) => {
...
>('expenses/updateExpense', async ({ id, updates }) => {
...
const expenseSlice = createSlice({
  ...
  reducers: {
    ...
    setExpenseType: (state, action: PayloadAction<'free' | 'budget'>) => {
      ...
      state.expenses =
        action.payload === 'free' ? state.freeExpenses : state.budgetExpenses;
    },
    ...
    updateExpenseOptimistic: (
      state,
      action: PayloadAction<{ id: string; updates: Partial<ExpenseItem> }>
    ) => {
      ...
      const freeIndex = state.freeExpenses.findIndex(
        (expense) => expense._id === id
      );
      if (freeIndex !== -1) {
        state.freeExpenses[freeIndex] = {
          ...state.freeExpenses[freeIndex],
          ...updates,
        };
      }

      const budgetIndex = state.budgetExpenses.findIndex(
        (expense) => expense._id === id
      );
      if (budgetIndex !== -1) {
        state.budgetExpenses[budgetIndex] = {
          ...state.budgetExpenses[budgetIndex],
          ...updates,
        };
      }
    ...
    deleteExpenseOptimistic: (state, action: PayloadAction<string>) => {
      ...
      state.freeExpenses = state.freeExpenses.filter(
        (expense) => expense._id !== id
      );
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(addExpense.fulfilled, (state, action) => {
        ...
        const tempIndex = state.expenses.findIndex(
          (expense) => expense._id && expense._id.startsWith('temp_')
        );
        if (tempIndex !== -1) {
        ...
        if (action.payload.type === 'free') {
          const freeTempIndex = state.freeExpenses.findIndex(
            (expense) => expense._id && expense._id.startsWith('temp_')
          );
          if (freeTempIndex !== -1) {
          ...
          const budgetTempIndex = state.budgetExpenses.findIndex(
            (expense) => expense._id && expense._id.startsWith('temp_')
          );
