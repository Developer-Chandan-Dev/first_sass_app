import { z } from 'zod';
import { sanitizeString, sanitizeNumber, isValidObjectId } from './input-sanitizer';
...
export const secureObjectIdSchema = z.string()
...
export function validateRequestBody<T>(schema: z.ZodSchema<T>, data: unknown): T {
  try {
    ...
    if (error instanceof z.ZodError) {
      throw new Error(`Validation failed: ${error.issues.map((e: z.ZodIssue) => e.message).join(', ')}`);
    }
...
export function validateSecurityHeaders(request: Request): boolean {
  ...
  const suspiciousPatterns = [
    ...
    'python-requests', 'curl/7.', 'wget/', 'libwww'
  ];
