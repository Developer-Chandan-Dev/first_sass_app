...
export function useIncomeTable() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      console.error('Delete income error:', sanitizeForLog(error instanceof Error ? error.message : 'Unknown error'));
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      console.error('Bulk delete incomes error:', sanitizeForLog(error instanceof Error ? error.message : 'Unknown error'));
    ...
    try {
      ...
      dataToExport.forEach((income, index) => {
        ...
        if (options.includeDescription) {
          rowData.push(sanitizeString(income.description || '-').substring(0, 20));
        }
        if (options.includeRecurring) {
          rowData.push(income.isRecurring ? `Yes${income.frequency ? ` (${sanitizeString(income.frequency || '')})` : ''}` : 'No');
        }
      ...
      console.error('PDF export error:', sanitizeForLog(error instanceof Error ? error.message : 'Unknown error'));
