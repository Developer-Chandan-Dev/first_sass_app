...
export function IncomeExpensesModal({
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      'download',
      `${incomeSource}-expenses-${new Date().toISOString().split('T')[0]}.csv`
    );
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(
        `Total Income: ₹${incomeAmount.toLocaleString()}`,
        margin + 5,
        yPos + 12
      );
      doc.text(
      ...
      doc.text(
        `Usage: ${((totalSpent / incomeAmount) * 100).toFixed(1)}%`,
        margin + 220,
        yPos + 12
      );
      yPos += 35;
      ...
      for (let i = 1; i <= totalPages; i++) {
        doc.setPage(i);
        doc.setFontSize(8);
        doc.text(`Page ${i} of ${totalPages}`, pageWidth - margin, 200, {
          align: 'right',
        });
      }

      doc.save(
        `${incomeSource}-expenses-${new Date().toISOString().split('T')[0]}.pdf`
      );
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              {expenses.length === 0
                ? 'No expenses found for this income source'
                ...
                <Table className="min-w-[600px]">
                  <TableHeader>
                    <TableRow className="bg-muted/50">
                      <TableHead className="w-24 sm:w-28 font-semibold">
                        Date
                      </TableHead>
                      <TableHead className="w-28 sm:w-32 font-semibold">
                        Category
                      </TableHead>
                      <TableHead className="min-w-[150px] sm:min-w-[200px] lg:min-w-[300px] font-semibold">
                        Description
                      </TableHead>
                      <TableHead className="w-20 sm:w-24 font-semibold">
                        Type
                      </TableHead>
                      <TableHead className="text-right w-24 sm:w-28 font-semibold">
                        Amount
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                      ...
                      >
                        ...
                        <TableCell className="py-4">
                          <div className="text-xs sm:text-sm text-foreground">
                            <div
                              className="max-w-[120px] sm:max-w-[180px] lg:max-w-[280px] break-words"
                        ...
                        <TableCell className="py-4">
                          <Badge
                            variant={
                              expense.type === 'budget' ? 'default' : 'outline'
                            }
                            className="text-xs whitespace-nowrap"
                          >
                            {expense.type}
