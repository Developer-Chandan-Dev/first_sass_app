  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await modalState.executeAsync(async () => {
      if (
        ...
        throw new Error(
          errors?.validation || 'Please fill in all required fields'
        );
      }
      ...
      const updates = {
        ...
        frequency: formData.isRecurring
          ? (formData.frequency as 'monthly' | 'weekly' | 'yearly')
          : undefined,
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {incomeTranslations?.editIncome || 'Edit Income'}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="source">
                {incomeTranslations?.source || 'Source'} *
              </Label>
              <Input
                id="source"
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={
                      incomeTranslations?.selectIncomeCategory ||
                      'Select category'
                    }
                  />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="Salary">
                    {incomeTranslations?.sources?.salary || 'Salary'}
                  </SelectItem>
                  <SelectItem value="Freelancing">
                    {incomeTranslations?.sources?.freelancing || 'Freelancing'}
                  </SelectItem>
                  <SelectItem value="Business">
                    {incomeTranslations?.sources?.business || 'Business'}
                  </SelectItem>
                  <SelectItem value="Investment">
                    {incomeTranslations?.sources?.investment || 'Investment'}
                  </SelectItem>
                  <SelectItem value="Rental">
                    {incomeTranslations?.sources?.rental || 'Rental'}
                  </SelectItem>
                  <SelectItem value="Commission">
                    {incomeTranslations?.sources?.commission || 'Commission'}
                  </SelectItem>
                  <SelectItem value="Bonus">
                    {incomeTranslations?.sources?.bonus || 'Bonus'}
                  </SelectItem>
                  <SelectItem value="Other">
                    {incomeTranslations?.sources?.other || 'Other'}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="description"
              placeholder={
                incomeTranslations?.briefDescriptionOfIncomeSource ||
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {incomeTranslations?.expensesWillReduceFromThisIncome ||
              ...
              <Switch
                ...
                onCheckedChange={(checked) =>
                  setFormData({ ...formData, isConnected: checked })
                }
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {incomeTranslations?.regularIncomeSource ||
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-4">
            ...
            >
              ...
              {modalState.isSubmitting
                ? 'Updating...'
                : incomeTranslations?.editIncome || 'Update Income'}
            </Button>
          </div>
        </form>
      </DialogContent>
