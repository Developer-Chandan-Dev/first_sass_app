...
const createIncomeSchema = (t: any) =>
  z.object({
    amount: z
      .number()
      .min(
        0.01,
        t.income?.form?.validation?.amountPositive ||
    ...
    source: z
      ...
      .min(
        1,
        t.income?.form?.validation?.sourceRequired || 'Source is required'
      ),
    category: z
      .string()
      .min(
        1,
        t.income?.form?.validation?.categoryRequired || 'Category is required'
      ),
    description: z
      .string()
      .min(
        1,
        t.income?.form?.validation?.descriptionRequired ||
  ...
  const onSubmit = async (data: IncomeFormData) => {
    await modalState.executeAsync(async () => {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || errors?.generic || 'Failed to add income'
        );
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-md mx-auto">
        <DialogHeader>
          <DialogTitle>
            {income?.addIncomeSource || 'Add Income Source'}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <Select onValueChange={(value) => setValue('category', value)}>
              <SelectTrigger>
                <SelectValue
                  placeholder={
                    income?.selectIncomeCategory || 'Select income category'
                  }
                />
              </SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder={
                income?.briefDescriptionOfIncomeSource ||
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {income?.expensesWillReduceFromThisIncome ||
