...
export function BudgetSavingsTracker({ budgets }: BudgetSavingsTrackerProps) {
  const calculateSavings = () => {
    ...
    const savingsRate = totalBudgeted > 0 ? (totalSavings / totalBudgeted) * 100 : 0;
    
    const completedBudgets = budgets.filter(b => b.status === 'completed');
    const completedSavings = completedBudgets.reduce((sum, b) => sum + Math.max(0, b.remaining), 0);
    
    const runningBudgets = budgets.filter(b => b.status === 'running');
    const potentialSavings = runningBudgets.reduce((sum, b) => sum + Math.max(0, b.remaining), 0);
  ...
  const getSavingsInsight = () => {
    if (savings.savingsRate >= 20) return { type: 'excellent', message: 'Excellent savings!' };
    if (savings.savingsRate >= 10) return { type: 'good', message: 'Good savings rate' };
    if (savings.savingsRate >= 5) return { type: 'average', message: 'Average savings' };
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xl font-bold">{formatCurrency(savings.totalBudgeted)}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Total Spent</p>
              <p className="text-xl font-bold">{formatCurrency(savings.totalSpent)}</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">{savings.savingsRate.toFixed(1)}%</span>
            </div>
            <Progress value={Math.min(savings.savingsRate, 100)} className="h-2" />
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            <div className="flex justify-between items-center p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
              <div>
                ...
                <p className="text-xs text-blue-600/80">{savings.completedBudgetsCount} budgets</p>
              </div>
              <div className="text-right">
                <p className="font-bold text-blue-600">{formatCurrency(savings.completedSavings)}</p>
            ...
            <div className="flex justify-between items-center p-3 bg-orange-50 dark:bg-orange-950/20 rounded-lg">
              <div>
                ...
                <p className="text-xs text-orange-600/80">{savings.runningBudgetsCount} budgets</p>
              </div>
              <div className="text-right">
                <p className="font-bold text-orange-600">{formatCurrency(savings.potentialSavings)}</p>
          ...
          <div className="pt-3 border-t">
            <div className="flex items-center gap-2 mb-2">
              <Badge variant={
                insight.type === 'excellent' ? 'default' :
                insight.type === 'good' ? 'secondary' :
                insight.type === 'average' ? 'outline' : 'destructive'
              }>
                {insight.type === 'excellent' && <TrendingUp className="h-3 w-3 mr-1" />}
                {insight.type === 'poor' && <TrendingDown className="h-3 w-3 mr-1" />}
                {insight.message}
              </Badge>
            </div>
