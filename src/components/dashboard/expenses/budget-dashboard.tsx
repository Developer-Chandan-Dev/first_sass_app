    ...
    <CollapsibleTrigger asChild>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {count !== undefined && (
            <Badge variant={variant === 'warning' ? 'destructive' : variant === 'success' ? 'default' : 'secondary'}>
              {count}
            </Badge>
          )}
        </div>
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-3">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={() => onEditBudget(budget)}>
        ...
        {budget.status === 'running' && (
          <div className="flex gap-2 mt-3">
            <Button size="sm" variant="outline" onClick={() => onStatusChange(budget._id, 'paused')}>
              Pause
            </Button>
            <Button size="sm" variant="outline" onClick={() => onStatusChange(budget._id, 'completed')}>
              Complete
            </Button>
          </div>
        )}
              ...
              <CollapsibleContent>
                <CardContent className="pt-0">
                  {exceededBudgets.map(budget => (
                    <div key={budget._id} className="p-3 bg-red-50 rounded-lg mb-2">
              ...
              <CardContent className="pt-0">
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
                    <h4 className="font-medium mb-2 text-blue-900 dark:text-blue-100">Spending Rate</h4>
                    <div className="text-2xl font-bold text-blue-700 dark:text-blue-300">
                      {totalBudgeted > 0 ? ((totalSpent / totalBudgeted) * 100).toFixed(1) : 0}%
                    </div>
                    <p className="text-sm text-muted-foreground">of total budget used</p>
                  </div>
                  <div className="p-4 bg-green-50 dark:bg-green-950/20 rounded-lg">
                    <h4 className="font-medium mb-2 text-green-900 dark:text-green-100">Savings Rate</h4>
                    <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                      {totalBudgeted > 0 ? (((totalBudgeted - totalSpent) / totalBudgeted) * 100).toFixed(1) : 0}%
                    </div>
                    <p className="text-sm text-muted-foreground">of budget saved</p>
                  </div>
                </div>
              </CardContent>
            </CollapsibleContent>
