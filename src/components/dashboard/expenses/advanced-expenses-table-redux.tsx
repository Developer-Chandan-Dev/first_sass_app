          ...
          {Array.from({ length: columnCount }).map((_, j) => (
            <TableCell
              ...
              className={
                j === 0
                  ? ''
                  : j >= 3 && j <= 4 && expenseType === 'free'
                    ? 'hidden sm:table-cell'
                    : j === 4 && expenseType === 'budget'
      ...
      <CardHeader className="pb-3">
        <div className="flex flex-col gap-4">
          <CardTitle className="text-lg sm:text-xl">
            {sanitizeString(expensesData?.title || 'Expenses')} ({totalCount})
          </CardTitle>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="outline" size="sm" className="w-auto">
                        ...
                        {sanitizeString(expensesData?.exportAll || 'Export')} (
                        {selectedRows.length})
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  <DropdownMenuItem onClick={() => handleCSVExport(false)}>
                    <FileText className="mr-2 h-4 w-4" />
                    {sanitizeString(
                      expensesData?.exportAllCSV || 'Export All CSV'
                    )}
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => {
                  ...
                  >
                    <FileText className="mr-2 h-4 w-4" />
                    {sanitizeString(
                      expensesData?.exportAllPDF || 'Export All PDF'
                    )}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            <Table>
                  ...
                  {expenseType === 'budget' && (
                    <TableHead className="hidden lg:table-cell min-w-[100px]">
                      <Button
                        ...
                        className="h-auto p-0 font-medium"
                      >
                        {sanitizeString(expensesData?.budget || 'Budget')}{' '}
                        {getSortIcon('budgetName')}
                      </Button>
                    </TableHead>
                  )}
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell
                      colSpan={expenseType === 'budget' ? 8 : 7}
                    ...
                    <TableRow key={expense._id}>
                      <TableCell className="py-2">
                        <Checkbox
                          ...
                          onCheckedChange={() =>
                            toggleRowSelection(expense._id)
                          }
                        />
                      </TableCell>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={() =>
                                handleDelete(expense._id, expensesData)
                              }
                            >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2 py-4">
          <div className="flex flex-col sm:flex-row items-center gap-2">
            <div className="text-xs sm:text-sm text-muted-foreground">
              ...
              {sanitizeString(expensesData?.to || 'to')}{' '}
              {Math.min(currentPage * pageSize, totalCount)}{' '}
              {sanitizeString(table?.of || 'of')} {totalCount}{' '}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="h-8">
                  {pageSize}{' '}
                  {sanitizeString(expensesData?.perPage || 'per page')}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent>
        ...
        expenses={
          isSelectedExport
            ? expenses.filter((expense: ExpenseItem) =>
                selectedRows.includes(expense._id)
              )
            : expenses
        }
