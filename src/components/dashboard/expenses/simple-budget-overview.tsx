...
import { Plus, Target, CheckCircle, Clock, Pause, Play, Edit, PiggyBank } from 'lucide-react';
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2">
              <Badge variant={budget.status === 'completed' ? 'default' : 'secondary'}>
                {budget.status === 'running' && <Clock className="h-3 w-3 mr-1" />}
                {budget.status === 'completed' && <CheckCircle className="h-3 w-3 mr-1" />}
                {budget.status === 'paused' && <Pause className="h-3 w-3 mr-1" />}
                {budget.status}
              </Badge>
              <Button
                size="sm"
      ...
      {budgets.length === 0 && (
        <Card className="text-center py-12">
          <CardContent>
            ...
            <h3 className="text-xl font-semibold mb-2">Start Your First Budget</h3>
