...
export function RecentActivityOverview() {
  ...
  const { freeExpenses, budgetExpenses, lastUpdated } = useAppSelector(state => state.expenses);

  useEffect(() => {
    // Fetch recent expenses from both types
    dispatch(fetchExpenses({ 
      expenseType: 'free', 
      filters: { period: 'all', category: '', startDate: '', endDate: '', search: '' }, 
      page: 1, 
      pageSize: 50 
    }));
    dispatch(fetchExpenses({ 
  ...
  if (allExpenses.length === 0) {
    return (
      <div className="space-y-4">
        {[
          { category: 'Food', amount: 450, time: '2 hours ago', color: 'bg-orange-500', type: 'free' },
          { category: 'Transport', amount: 120, time: '5 hours ago', color: 'bg-blue-500', type: 'budget' },
          { category: 'Shopping', amount: 2300, time: '1 day ago', color: 'bg-purple-500', type: 'free' }
        ].map((activity, index) => (
          <div key={index} className="flex items-center justify-between">
            ...
            <div className="text-right">
              <p className="font-medium text-red-600">-₹{activity.amount.toLocaleString()}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div key={expense._id} className="flex items-center gap-2 w-full min-w-0">
          ...
          <div className="flex-1 min-w-0 overflow-hidden">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <div className={`w-2 h-2 rounded-full ${getExpenseTypeColor(expense.type)}`} 
                   title={expense.type === 'budget' ? 'Budget Expense' : 'Free Expense'}>
              </div>
            </div>
          </div>
