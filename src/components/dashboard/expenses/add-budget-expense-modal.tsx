...
const createExpenseSchema = (t: ReturnType<typeof useDashboardTranslations>) =>
  z.object({
    amount: z
      .number()
      .min(0.01, t.expenses.form.validation.amountGreaterThanZero),
...
export function AddBudgetExpenseModal({
  open,
  onOpenChange,
  onExpenseAdded,
  ...
  const onSubmit = async (data: ExpenseFormData) => {
    ...
    if (data.amount > selectedBudget.remaining) {
      ...
      if (
        !confirm(
          `This expense exceeds your budget by ₹${exceed.toLocaleString()}. Continue anyway?`
        )
      ) {
        return;
      }
        ...
        <div className="flex-1 overflow-y-auto pr-2">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              >
                <option value="">
                  {t.expenses.selectIncomeSource.replace('Income', 'Budget')}
                </option>
                {budgets.map((budget) => (
                  <option key={budget._id} value={budget._id}>
                    {budget.name} - ₹{budget.remaining.toLocaleString()}{' '}
                    {t.expenses.remaining.toLowerCase()}
                  </option>
                ))}
              </select>
              {errors.budgetId && (
            ...
            {selectedBudget && (
              <div className="p-3 bg-muted rounded-lg">
                ...
                <div className="text-sm text-muted-foreground">
                  ₹{selectedBudget.spent.toLocaleString()} {t.expenses.spent} •
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="affectsBalance" className="text-sm">
                {t.expenses.reduceFromBalance}
              </Label>
              <Switch
                id="affectsBalance"
            ...
            {watchedAffectsBalance && (
              <div>
                <Label htmlFor="incomeId" className="text-sm">
                  {t.expenses.selectIncomeSource}
                </Label>
                <select
                  id="incomeId"
                  {...register('incomeId')}
                ...
                >
                  <option value="">
                    {t.expenses.chooseIncomeToReduceFrom}
                  </option>
            ...
            {watchedIsRecurring && (
              <div>
                ...
                >
                  ...
                  <option value="weekly">
                    {t.expenses.frequencies.weekly}
                  </option>
                  <option value="monthly">
                    {t.expenses.frequencies.monthly}
                  </option>
                  <option value="yearly">
                    {t.expenses.frequencies.yearly}
                  </option>
                </select>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-4">
              <Button
                type="submit"
                disabled={isSubmitting || !selectedBudget}
