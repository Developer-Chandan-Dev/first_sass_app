...
interface RunningBudgetsProps {
  budgets: Budget[];
  onToggleBudget: (budgetId: string, action: 'pause' | 'resume' | 'complete') => void;
  onEditBudget: (budget: Budget) => void;
  onDeleteBudget: (budgetId: string) => void;
}

export function RunningBudgets({ budgets, onToggleBudget, onEditBudget, onDeleteBudget }: RunningBudgetsProps) {
              ...
              {savings > 0 && (
                <div className="bg-green-50 dark:bg-green-950/20 p-2 rounded text-sm">
                  <div className="flex items-center gap-1 text-green-600">
                    ...
                    <span className="font-medium">Potential Savings: {formatCurrency(savings)}</span>
                  </div>
                </div>
              )}
            </div>
